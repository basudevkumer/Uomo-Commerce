...
const navigation = [
  {
    ...
    items: [
      ...
      ["My Orders", "/dashboard/order", ShoppingBag, "3"],
      ["Wishlist", "/dashboard/wishlist", Star, "8"],
      ["Addresses", "/dashboard/address", Truck],
      ["Account Details", "/dashboard/account-details", Settings],
    ],
  },
  {
    label: "SHOPPING",
    items: [
      ["Cart", "/dashboard/cart", ShoppingCart],
  ...
  },
  {
    label: "SUPPORT",
    items: [
      ["Help Center", "/contact", HelpCircle],
      ["Contact Support", "/contact", MessageCircle],
    ],
  },
];
      ...
      >
        ...
        <div className="custom-scrollbar flex min-h-0 flex-1 flex-col justify-between overflow-y-auto px-4 py-7">
          <nav
            ...
            className="custom-scrollbar min-h-0 flex-1 space-y-6 overflow-y-auto"
          >
            {navigation.map(({ label, items }) => (
              <div key={label} className="space-y-1.5">
                ...
                {items.map(([itemLabel, href, Icon, count]) => {
                  ...
                  const itemCount = itemLabel === "Wishlist" ? wishlistCount : count;
                  return (
                    <Link
                      key={`${itemLabel}-${href}`}
                      href={href}
                      ...
                      className={`group flex min-h-[43px] items-center justify-between rounded-[14px] px-4 text-[13px] font-medium transition-all ${active ? "bg-red text-white shadow-[0_8px_20px_rgba(214,0,28,0.16)]" : "text-second hover:bg-secondbg hover:text-head"}`}
                    >
                      ...
                      {itemCount > 0 && (
                        <span
                          className={`min-w-5 rounded-full px-1.5 py-1 text-center text-[10px] font-semibold ${active ? "bg-white/18 text-white" : "bg-secondbg text-second"}`}
                        >
                          {itemCount}
                        </span>
                      )}
          ...
          <div className="space-y-1.5">
            <Link
              href="/logout"
              className="flex items-center gap-3.5 rounded-[14px] px-4 py-3 text-[13px] font-medium text-second hover:bg-secondbg hover:text-head"
            >
              <UserRound size={18} strokeWidth={1.8} /> Sign out
            </Link>
            <div className="mt-5 border-t border-footer px-3 pt-5">
              <div className="flex items-center gap-3">
                <div className="flex size-10 shrink-0 items-center justify-center rounded-full bg-secondbg text-[12px] font-bold text-head">
                  JD
                </div>
                <div className="min-w-0">
                  <p className="truncate text-[13px] font-semibold text-head">
                    Jamie Doe
                  </p>
                  <p className="truncate text-[12px] text-second">
                    jamie@example.com
                  </p>
                </div>
              </div>
    ...
    <header className="sticky top-0 z-20 flex h-[76px] items-center justify-between border-b border-footer bg-white/95 px-4 backdrop-blur sm:px-6 lg:px-9">
      <div className="flex items-center gap-3">
        ...
        <div className="hidden h-10 w-[290px] items-center gap-3 rounded-xl bg-secondbg px-3.5 text-[13px] text-second sm:flex">
          ...
          <kbd className="ml-auto rounded-md border border-footer bg-white px-1.5 py-0.5 text-[10px] text-second">
            ⌘ K
          </kbd>
        </div>
        <button
          className="rounded-xl p-2 text-second hover:bg-secondbg sm:hidden"
          aria-label="Search"
        >
          <Search size={20} />
        </button>
      ...
      <div className="flex items-center gap-3 sm:gap-5">
        <button
          ...
          className="relative rounded-xl p-2 text-second hover:bg-secondbg"
        >
          <Bell size={19} />
        ...
        <button className="flex items-center gap-2 rounded-xl px-1.5 py-1.5 hover:bg-secondbg">
          <span className="flex size-8 items-center justify-center rounded-full bg-head text-[10px] font-bold text-white">
            JD
          </span>
          <span className="hidden text-left sm:block">
            <span className="block text-[12px] font-semibold text-head">
              Jamie Doe
            </span>
            <span className="block text-[10px] text-second">Member</span>
          </span>
